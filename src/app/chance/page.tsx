...
const SCHOOLS_FALLBACK: School[] = [
  ...
  { id: 7, name: "Penn Perelman SOM", abbr: "PENN", state: "PA", type: "MD", medianGPA: 3.9, medianMCAT: 523, acceptanceRate: 3.3, inStateTX: false, utApplied: 22, utAccepted: 1 },
  ...
  { id: 32, name: "Albany Medical College", abbr: "AMC", state: "NY", type: "MD", medianGPA: 3.6, medianMCAT: 511, acceptanceRate: 12.1, inStateTX: false, utApplied: 10, utAccepted: 1 },
...
export default function ChancePage() {
  ...
  const enriched = useMemo(
    () =>
      schools
        .map((s) => {
          const probability = calcProbability(gpa, mcat, s);
          return { ...s, probability, tier: tierFromProb(probability) };
        })
      ...
      <div className="space-y-3">
        ...
        <div className="grid gap-1.5" style={{ gridTemplateColumns: "repeat(10, minmax(0, 1fr))" }}>
          {pageSchools.map((s) => (
            ...
            >
              <span className="text-[9px] font-mono font-bold text-white/70 leading-none tracking-wide">{s.abbr}</span>
              <span className="text-xs font-bold tabular-nums leading-none" style={{ color: s.probability >= 28 ? "#fff" : "#fca5a5" }}>
                {s.probability}%
              </span>
            </button>
          ))}
        ...
        <div className="flex items-center gap-4 px-4 py-3 bg-white/[0.04] border border-white/10 rounded-xl min-h-[52px]">
          {hovered ? (
            <>
              <div className="w-3 h-3 rounded-sm shrink-0" style={{ backgroundColor: probBg(hovered.probability) }} />
              <p className="text-sm text-white/70 flex-1">
                <span className="font-semibold text-white">{hovered.name}</span>
                {" · "}Median GPA <span className="font-mono text-white">{hovered.medianGPA.toFixed(2)}</span>
                {" · "}Median MCAT <span className="font-mono text-white">{hovered.medianMCAT}</span>
                {" · "}National acceptance <span className="font-mono text-white">{hovered.acceptanceRate}%</span>
              </p>
              <span className="text-sm font-bold font-mono shrink-0" style={{ color: probColor(hovered.probability) }}>
            ...
            <>
              ...
              <p className="text-sm text-white/55 flex-1">
                Approximately <span className="font-semibold text-white">{utStat.applied}</span> UT students with a similar profile applied last cycle.{" "}
                <span className="font-semibold text-white">{utStat.accepted}</span> were accepted.
              </p>
              <span className="text-xs text-white/25 font-mono shrink-0">Hover a cell for details</span>
            </>
          )}
        </div>
            ...
            <div className="flex gap-1.5">
              {Array.from({ length: totalPages }).map((_, i) => (
                <button
                  key={i}
                  onClick={() => setPage(i)}
                  className={`w-2 h-2 rounded-full transition-colors ${i === page ? "bg-[#BF5700]" : "bg-white/20 hover:bg-white/40"}`}
                />
              ))}
            </div>
