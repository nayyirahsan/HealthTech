...
interface School {
  ...
  utAcceptRate: number | null;
  tuition: number;
  classSize: number;
  overview: string;
  secondaries: { prompt: string; wordLimit: number | null }[];
  prerequisites: string[];
  utOutcomes: Record<string, Record<string, UTOutcomeCell>>;
}
...
function BiasBadge({ bias }: { bias: School["inStateBias"] }) {
  ...
  return <span className={`text-[10px] px-1.5 py-0.5 rounded border font-medium ${cfg}`}>{bias}</span>;
}
              ...
              {DRAWER_TABS.map((t) => (
                <button
                  ...
                  className={`px-3 py-2 text-xs font-medium border-b-2 transition-colors ${
                    tab === t ? "border-[#BF5700] text-[#BF5700]" : "border-transparent text-white/35 hover:text-white/60"
                  }`}
                >
                  {t}
                </button>
            ...
            {tab === "UT Outcomes" && (
              <div className="space-y-4">
                <p className="text-xs text-white/40 leading-relaxed">
                  UT Austin applicant outcomes by GPA × MCAT band. Data from HPO reports 2021–2023. Cell = applied / interviewed /{" "}
                  ...
                  <table className="w-full text-xs border-collapse">
                    <thead>
                      <tr>
                        ...
                        {MCAT_BANDS.map((b) => (
                          <th key={b} className="text-center py-2 px-1 text-white/40 font-mono font-medium">
                            {b}
                          </th>
                        ))}
                      </tr>
                          ...
                          {MCAT_BANDS.map((mcatBand) => {
                            ...
                            return (
                              <td key={mcatBand} className="py-1 px-1">
                                <div className="heat-cell rounded-md p-2 text-center min-w-[72px]" style={{ backgroundColor: heatColor(cell.accepted, cell.applied) }}>
                                  {cell.applied === 0 ? (
                                    <span className="text-white/15">—</span>
                                  ) : (
                                    <div className="space-y-0.5">
                                      <p className="text-white/50">
                                        {cell.applied} / {cell.interviewed}
                                      </p>
...
function ColHeader({
  ...
  return (
    <th className={`px-4 py-3 text-left cursor-pointer select-none whitespace-nowrap ${className}`} onClick={() => onSort(sortKey)}>
...
export default function SchoolsPage() {
  ...
  const rows = useMemo(() => {
    ...
    return list;
  }, [schools, search, typeFilter, stateFilter, gpaRange, mcatRange, sortKey, sortDir]);
    ...
    <div className="min-h-full bg-[#0F172A] flex flex-col">
      <div className="px-6 pt-6 pb-4 border-b border-white/[0.07]">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-white/35 mt-0.5">
              {loading ? "Loading schools…" : `${rows.length} of ${schools.length} schools · Click any row for details`}
            </p>
          </div>
        ...
        <div className="flex flex-wrap items-end gap-5">
              ...
              {(["All", "MD", "DO"] as const).map((t) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                    typeFilter === t ? "bg-[#BF5700]/20 border-[#BF5700]/40 text-[#BF5700]" : "bg-white/[0.04] border-white/10 text-white/40 hover:text-white/70"
                  }`}
                >
                  {t}
                </button>
            ...
            <div className="flex gap-1">
              {(["All", "In-State", "Out-of-State"] as const).map((s) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                    stateFilter === s ? "bg-[#BF5700]/20 border-[#BF5700]/40 text-[#BF5700]" : "bg-white/[0.04] border-white/10 text-white/40 hover:text-white/70"
                  }`}
                >
                  {s}
                </button>
          ...
          <RangeFilter label="GPA Range" min={2.5} max={4.0} step={0.05} value={gpaRange} onChange={setGpaRange} />
          <RangeFilter label="MCAT Range" min={472} max={528} step={1} value={mcatRange} onChange={setMcatRange} />
            ...
            {rows.length === 0 ? (
              ...
              visibleRows.map((school, i) => (
                <tr
                  ...
                  className={`border-b border-white/[0.05] cursor-pointer transition-colors hover:bg-[#BF5700]/[0.06] ${i % 2 === 0 ? "bg-transparent" : "bg-white/[0.015]"}`}
                >
                  <td className="px-4 py-3 pl-6">
                    <div>
                      <p className="text-sm font-medium text-white/85">{school.name}</p>
                      <p className="text-[11px] text-white/30 mt-0.5 font-mono">
                        {school.abbr} · {school.state}
                      </p>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 font-mono text-sm text-white/70 tabular-nums">{school.medianGPA.toFixed(2)}</td>
                  <td className="px-4 py-3 font-mono text-sm text-white/70 tabular-nums">{school.medianMCAT}</td>
                  <td className="px-4 py-3 font-mono text-sm text-white/70 tabular-nums">{school.acceptanceRate}%</td>
                  <td className="px-4 py-3">
                    <BiasBadge bias={school.inStateBias} />
                  </td>
                  <td className="px-4 py-3 pr-6 font-mono text-sm tabular-nums">
                    {school.utAcceptRate != null ? <span className="text-[#BF5700]">{school.utAcceptRate}%</span> : <span className="text-white/20">—</span>}
                  </td>
                </tr>
              ))
            )}
